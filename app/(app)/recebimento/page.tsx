...
export default function ReceivingPage() {
  ...
  const products = useMemo(() => state.products.filter((product) => product.kind !== "combo" && !isDoseShortcut(product) && [product.name, product.barcode, product.sku, product.category, product.brand].join(" ").toLowerCase().includes(query.toLowerCase())).slice(0, 24), [state.products, query]);
    ...
    <div className="mt-6 grid gap-6 2xl:grid-cols-[1fr_450px]">
      <section className="space-y-4">
        <div className="panel p-4 md:p-5">
          ...
          <form onSubmit={(event) => { event.preventDefault(); processScan(); }}><label><span className="mb-2 flex items-center gap-2 text-sm font-bold"><ScanBarcode size={19} className="text-lime" /> Bipe os produtos recebidos</span><div className="grid gap-2 sm:grid-cols-[96px_1fr_auto]"><NumberInput className="input h-16 text-center text-xl font-black" min={1} max={9999} step={1} emptyWhenZero={false} value={scanQty} onValueChange={setScanQty} title="Quantidade do próximo bip" /><div className="relative"><Barcode className="absolute left-4 top-1/2 -translate-y-1/2 text-brand" size={24} /><input ref={scannerRef} className="input h-16 pl-12 text-xl font-mono" value={scan} onChange={(event) => setScan(event.target.value)} placeholder="Bipe aqui" autoComplete="off" /></div><button className="btn-primary px-6">Somar</button></div></label></form>
        </div>
