    ...
    <div className="grid gap-6 2xl:grid-cols-[1fr_430px]">
      ...
      <aside className="panel h-fit overflow-hidden 2xl:sticky 2xl:top-24">
        ...
        <div className="space-y-3 border-t border-line bg-black/10 p-4">
          ...
          <div className="grid grid-cols-2 gap-3"><label><span className="mb-1 block text-xs text-slate-500">Desconto (F5)</span><NumberInput ref={discountInput} className="input" min={0} max={totals.subtotal} step={0.01} value={discount} onValueChange={setDiscount} placeholder="0,00" /></label><div className="rounded-xl border border-line bg-white/[0.03] p-3"><p className="text-xs text-slate-500">Itens</p><p className="mt-1 text-lg font-black">{cart.reduce((sum, item) => sum + item.quantity, 0)}</p></div></div>
