...
export default function SettingsPage() {
  const { state, dataMode, syncStatus, updateCompany, updateScannerSettings, updateCurrentOperator, exportBackup, importBackup, resetDemo } = useStore();
  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <section className="panel p-5">
        ...
        <div className="space-y-4"><label><span className="mb-1.5 block text-sm font-semibold">Nome do operador</span><input className="input" value={operatorName} disabled={dataMode === "supabase"} onChange={(e) => setOperatorName(e.target.value)} /></label><label><span className="mb-1.5 block text-sm font-semibold">Perfil</span><select className="select" value={role} disabled={dataMode === "supabase"} onChange={(e) => setRole(e.target.value as UserRole)}>{roles.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><p className="rounded-xl border border-line bg-white/[0.03] p-3 text-xs leading-5 text-slate-400">{dataMode === "supabase" ? "No modo Supabase, nome e perfil vêm do usuário autenticado e não são simulados nesta tela." : roles.find((item) => item.value === role)?.description}</p>{dataMode === "local" && <button className="btn-primary" onClick={() => { if (!operatorName.trim()) return toast.error("Informe o nome do operador."); updateCurrentOperator({ name: operatorName.trim(), role }); toast.success("Perfil operacional atualizado. O menu foi simplificado para este usuário."); }}><ShieldCheck size={18} /> Aplicar perfil</button>}</div>
      </section>
    </div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      <section className="panel p-5"><div className="mb-5"><h2 className="section-title">Persistência e backup</h2><p className="muted">{dataMode === "supabase" ? `Nuvem ativa · ${syncStatus === "synced" ? "dados sincronizados" : syncStatus === "saving" ? "salvando alterações" : "verifique o indicador de sincronização"}.` : "Modo local: os dados ficam salvos neste navegador."}</p></div><div className="space-y-3"><button className="btn-lime w-full" onClick={() => saveFile(`backup-bebs-${new Date().toISOString().slice(0, 10)}.json`, exportBackup())}><Download size={18} /> Baixar backup</button><button className="btn-ghost w-full" onClick={() => file.current?.click()}><Upload size={18} /> Restaurar backup</button><input ref={file} type="file" accept="application/json" hidden onChange={async (e) => { const selected = e.target.files?.[0]; if (!selected) return; try { importBackup(await selected.text()); toast.success("Backup restaurado e colocado na fila de gravação."); } catch (error) { toast.error(error instanceof Error ? error.message : "Backup inválido."); } e.currentTarget.value = ""; }} />{dataMode === "local" ? <button className="btn-danger w-full" onClick={() => { if (window.confirm("Restaurar os dados iniciais da Beb's? Os dados atuais deste navegador serão substituídos.")) { resetDemo(); setCompany(state.company); toast.success("Base inicial restaurada."); } }}><RefreshCw size={18} /> Restaurar base inicial</button> : <div className="rounded-xl border border-lime/20 bg-lime/5 p-3 text-xs leading-5 text-slate-300">No modo Supabase, o reset da demonstração fica bloqueado para evitar apagar a operação real por engano. Cada alteração confirmada também recebe proteção local imediata antes de ser enviada para a nuvem.</div>}</div></section>
