...
const DRAFT_KEY = "bebs-pdv-draft-v214";
...
export default function PosPage() {
  ...
  const [doseStep, setDoseStep] = useState<"choice" | "ml">("choice");
  const [doseMl, setDoseMl] = useState(0);
  const [dosePrice, setDosePrice] = useState(0);
  const [doseDisplayName, setDoseDisplayName] = useState("");
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "F5" && cart.length) { event.preventDefault(); openPayment(); window.setTimeout(() => discountInput.current?.focus(), 120); }
  ...
  const addItem = (product: Product, mode: CartItem["mode"] = product.kind === "combo" ? "combo" : "unit", doseAmountMl?: number, unitPrice?: number, quantity = 1, displayName?: string) => {
    const qty = Math.max(1, Math.floor(quantity || 1));
    const safePrice = Math.max(0, Number(unitPrice ?? product.price));
    const key = mode === "dose"
      ? `${product.id}_${mode}_${doseAmountMl || 0}_${safePrice.toFixed(2)}`
      : `${product.id}_${mode}_${doseAmountMl || 0}`;
    const found = cart.find((item) => item.id === key);
    const label = displayName || product.name;
    const nextItem: CartItem = found
      ? { ...found, quantity: found.quantity + qty }
      : { id: key, productId: product.id, name: label + (doseAmountMl ? ` · ${doseAmountMl} ml` : ""), mode, quantity: qty, unitPrice: safePrice, doseMl: doseAmountMl };
    const projected = found ? cart.map((item) => item.id === key ? nextItem : item) : [...cart, nextItem];
    try {
      applyCartToStock(state.products, projected);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Estoque insuficiente para esta venda.");
      focusScanner();
      return false;
    }
    setCart(projected);
    toast.success(mode === "dose" ? `${doseAmountMl} ml de ${label} adicionados.` : `${qty}× ${product.name} adicionado.`);
    focusScanner();
    return true;
  };
  ...
  const openBottleSale = (product: Product) => {
    setDoseProduct(product);
    setDoseDisplayName(product.name);
    setDoseStep("choice");
    setDoseMl(0);
    setDosePrice(0);
  };

  const openDoseShortcut = (shortcut: Product) => {
    const source = state.products.find((product) => product.id === shortcut.doseSourceProductId && product.kind === "volume");
    if (!source) { toast.error("A garrafa vinculada a este Produto Dose não foi encontrada."); return; }
    setDoseProduct(source);
    setDoseDisplayName(shortcut.name);
    setDoseStep("ml");
    setDoseMl(0);
    setDosePrice(0);
  };
  ...
  const handleProductClick = (product: Product) => {
    if (isDoseShortcut(product)) return openDoseShortcut(product);
    if (product.kind === "volume") return openBottleSale(product);
    addItem(product);
  };
  ...
  const changeQty = (id: string, delta: number) => {
    ...
    try { applyCartToStock(state.products, projected); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Estoque insuficiente."); return; }
    setCart(projected);
  };
  ...
  return <>
    ...
    <div className="space-y-5 pb-24">
      ...
      <div className="panel p-4 md:p-5">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="relative max-w-2xl flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={18} /><input ref={searchInput} className="input h-12 pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar nome, código, SKU, marca ou categoria (F2)" /></div><span className="text-xs text-slate-500">{visible.length} resultados</span></div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{visible.map((product) => <button key={product.id} onClick={() => handleProductClick(product)} className="panel-soft group min-h-28 p-4 text-left transition hover:-translate-y-0.5 hover:border-brand/50 hover:bg-brand/[0.04]"><div className="flex items-start justify-between gap-3"><div className={`rounded-xl p-2.5 ${product.kind === "volume" ? "bg-amber-500/10 text-amber-300" : product.kind === "combo" ? "bg-violet-500/10 text-violet-300" : "bg-brand/10 text-brand"}`}>{product.kind === "volume" ? <Wine size={20} /> : product.kind === "combo" ? <PackagePlus size={20} /> : <Beer size={20} />}</div><span className="text-xs text-slate-500">{isDoseShortcut(product) ? "Produto Dose" : product.kind === "combo" ? "Combo" : product.kind === "volume" ? `${product.stock} fechada(s)` : `${product.stock} un.`}</span></div><p className="mt-3 line-clamp-2 font-bold">{product.name}</p><div className="mt-2 flex items-end justify-between gap-3"><span className="truncate text-xs text-slate-500">{product.location || product.category}</span><span className="text-lg font-black text-lime">{isDoseShortcut(product) ? "Valor na hora" : currency(product.price)}</span></div></button>)}</div>
      </div>

      {cartPreviewOpen && <section className="panel p-4 md:p-5"><div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><ShoppingCart className="text-brand" size={21} /><h2 className="text-xl font-black">Venda atual</h2></div><button className="btn-ghost" onClick={() => setCartPreviewOpen(false)}>Fechar</button></div>{!cart.length ? <EmptyState icon={ShoppingCart} title="Carrinho vazio" description="Bipe um produto ou toque em um favorito." /> : <div className="grid gap-2 lg:grid-cols-2">{cart.map((item) => <div key={item.id} className="panel-soft flex items-center gap-3 p-3"><div className="min-w-0 flex-1"><p className="truncate font-semibold">{item.name}</p><p className="mt-1 text-sm text-lime">{currency(item.unitPrice)} · total {currency(item.unitPrice * item.quantity)}</p></div><div className="flex items-center rounded-lg border border-line"><button className="p-2 hover:bg-white/5" onClick={() => changeQty(item.id, -1)}><Minus size={14} /></button><span className="min-w-9 text-center text-sm font-bold">{item.quantity}</span><button className="p-2 hover:bg-white/5" onClick={() => changeQty(item.id, 1)}><Plus size={14} /></button></div><button onClick={() => setCart((items) => items.filter((row) => row.id !== item.id))} className="rounded-lg p-2 text-red-300 hover:bg-red-500/10"><Trash2 size={16} /></button></div>)}</div>}</section>}
    </div>
    ...
    <Modal open={Boolean(doseProduct)} onClose={closeDoseModal} title={doseProduct ? (doseStep === "choice" ? `Como vender ${doseProduct.name}?` : `Venda em ml · ${doseDisplayName || doseProduct.name}`) : "Venda por ml"} width="max-w-lg">
      {doseProduct && <div className="space-y-4">
        <div className="panel-soft p-4">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs text-slate-500">Garrafa de origem</p><p className="font-bold">{doseProduct.name}</p></div><span className="badge border-lime/30 bg-lime/10 text-lime">{Math.round(availableVolumeMl(doseProduct))} ml disponíveis</span></div>
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg border border-line bg-black/20 p-2"><span className="text-slate-500">Fechadas</span><p className="mt-1 font-black">{doseProduct.stock}</p></div><div className="rounded-lg border border-line bg-black/20 p-2"><span className="text-slate-500">Aberta</span><p className="mt-1 font-black">{Math.round(doseProduct.openVolumeMl || 0)} ml</p></div></div>
        </div>
        {doseStep === "choice" ? <>
          <button className="btn-ghost h-14 w-full justify-between" onClick={() => { if (addItem(doseProduct, "unit")) closeDoseModal(); }}><span>Vender garrafa inteira</span><strong>{currency(doseProduct.price)}</strong></button>
          <button className="btn-primary h-14 w-full justify-between" onClick={() => { setDoseStep("ml"); setDoseMl(0); setDosePrice(0); }}><span>Vender em ml</span><strong>Informar dose</strong></button>
        </> : <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (doseMl <= 0) return toast.error("Informe quantos ml foram servidos."); if (dosePrice <= 0) return toast.error("Informe o valor cobrado pela dose."); const ok = addItem(doseProduct, "dose", doseMl, dosePrice, 1, doseDisplayName || doseProduct.name); if (ok) closeDoseModal(); }}>
          <div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-semibold">Quantidade servida (ml)</span><NumberInput autoFocus className="input h-14 text-xl font-black" min={1} max={availableVolumeMl(doseProduct)} step={1} value={doseMl} onValueChange={setDoseMl} placeholder="Ex.: 70" /></label><label><span className="mb-1.5 block text-sm font-semibold">Valor desta dose (R$)</span><NumberInput className="input h-14 text-xl font-black" min={0.01} step={0.01} value={dosePrice} onValueChange={setDosePrice} placeholder="Ex.: 25,00" /></label></div>
          <div className="rounded-xl border border-cyan-400/20 bg-cyan-400/[0.04] p-3 text-xs text-slate-300">Cada venda pode ter <strong className="text-white">ml e valor diferentes</strong>. Ao confirmar, o sistema usa primeiro a garrafa aberta e abre a próxima automaticamente quando necessário.</div>
          <div className="grid grid-cols-2 gap-2">{doseDisplayName === doseProduct.name && <button type="button" className="btn-ghost" onClick={() => setDoseStep("choice")}>Voltar</button>}<button className="btn-lime col-span-1 h-12">Adicionar {doseMl > 0 ? `${doseMl} ml` : "dose"}</button></div>
        </form>}
      </div>}
    </Modal>

    <Modal open={paymentOpen} onClose={() => { setPaymentOpen(false); focusScanner(); }} title="Checkout da venda" width="max-w-6xl">
      <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="space-y-4">
          <div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-slate-500">Conferência</p><h3 className="text-xl font-black">Itens da venda</h3></div>{cart.length > 0 && <button className="text-sm font-semibold text-red-300" onClick={() => { if (window.confirm("Limpar a venda atual?")) setCart([]); }}>Limpar</button>}</div>
          <div className="max-h-[48vh] space-y-2 overflow-y-auto pr-1">{cart.map((item) => <div key={item.id} className="panel-soft flex items-center gap-3 p-3"><div className="min-w-0 flex-1"><p className="truncate font-semibold">{item.name}</p><p className="mt-1 text-sm text-lime">{currency(item.unitPrice)} · {currency(item.unitPrice * item.quantity)}</p></div><div className="flex items-center rounded-lg border border-line"><button className="p-2 hover:bg-white/5" onClick={() => changeQty(item.id, -1)}><Minus size={14} /></button><span className="min-w-10 text-center text-sm font-bold">{item.quantity}</span><button className="p-2 hover:bg-white/5" onClick={() => changeQty(item.id, 1)}><Plus size={14} /></button></div><button onClick={() => setCart((items) => items.filter((row) => row.id !== item.id))} className="rounded-lg p-2 text-red-300 hover:bg-red-500/10"><Trash2 size={16} /></button></div>)}</div>
          <div className="grid gap-3 sm:grid-cols-2">
            <button className="flex items-center justify-between rounded-xl border border-line bg-white/[0.03] p-3 text-left" onClick={() => setCustomerOpen(true)}><div className="flex items-center gap-2"><UserRound size={17} className="text-brand" /><div><p className="text-xs text-slate-500">Cliente</p><p className="text-sm font-semibold">{selectedCustomer?.name || "Consumidor final"}</p></div></div><span className="text-xs text-brand">Alterar</span></button>
            <label><span className="mb-1 block text-xs text-slate-500">Desconto</span><NumberInput ref={discountInput} className="input h-12" min={0} max={totals.subtotal} step={0.01} value={discount} onValueChange={setDiscount} placeholder="0,00" /></label>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-xl border border-line bg-white/[0.02] p-3"><p className="text-xs text-slate-500">Itens</p><p className="mt-1 text-lg font-black">{cart.reduce((sum, item) => sum + item.quantity, 0)}</p></div><div className="rounded-xl border border-line bg-white/[0.02] p-3"><p className="text-xs text-slate-500">Subtotal</p><p className="mt-1 text-lg font-black">{currency(totals.subtotal)}</p></div><div className="rounded-xl border border-line bg-white/[0.02] p-3"><p className="text-xs text-slate-500">Desconto</p><p className="mt-1 text-lg font-black text-amber-300">-{currency(discount)}</p></div></div>
        </section>

        <section className="space-y-4 rounded-2xl border border-line bg-black/10 p-4 md:p-5">
          <div className="rounded-2xl border border-lime/25 bg-lime/[0.05] p-5 text-center"><p className="text-xs font-bold uppercase tracking-widest text-slate-500">Total a receber</p><p className="mt-1 text-4xl font-black text-lime">{currency(totals.total)}</p></div>
          <div><p className="mb-2 text-sm font-bold">Forma de pagamento</p><div className="grid grid-cols-2 gap-2">{methods.map((method) => <button key={method} type="button" className={paymentLines.length === 1 && paymentLines[0]?.method === method ? "btn-primary h-12" : "btn-ghost h-12"} onClick={() => { setPaymentLines([{ method, amount: totals.total }]); setCashReceived(totals.total); }}>{methodIcon(method)} {method}</button>)}</div></div>
          <div className="space-y-2">{paymentLines.map((line, index) => <div key={`${line.method}-${index}`} className="grid grid-cols-[1fr_130px_auto] gap-2"><select className="select" value={line.method} onChange={(event) => setPaymentLines((lines) => lines.map((item, i) => i === index ? { ...item, method: event.target.value as PaymentMethod } : item))}>{[...methods, "Outro" as PaymentMethod].map((method) => <option key={method}>{method}</option>)}</select><NumberInput className="input text-right font-bold" min={0} step={0.01} value={line.amount} onValueChange={(value) => setPaymentLines((lines) => lines.map((item, i) => i === index ? { ...item, amount: value } : item))} /><button className="btn-danger px-3" type="button" disabled={paymentLines.length === 1} onClick={() => setPaymentLines((lines) => lines.filter((_, i) => i !== index))}><Trash2 size={16} /></button></div>)}</div>
          <div className="flex flex-wrap items-center gap-2"><button className="btn-ghost" type="button" onClick={() => { const used = paymentLines.reduce((sum, line) => sum + line.amount, 0); setPaymentLines((lines) => [...lines, { method: "Dinheiro", amount: Math.max(0, totals.total - used) }]); }}><Plus size={16} /> Dividir pagamento</button>{Math.abs(paymentDifference) > 0.009 && <span className={`badge ${paymentDifference > 0 ? "text-amber-300" : "text-red-300"}`}>{paymentDifference > 0 ? "Falta" : "Excede"} {currency(Math.abs(paymentDifference))}</span>}</div>
          {hasCash && <div className="grid gap-3 rounded-xl border border-line bg-white/[0.03] p-4 sm:grid-cols-2"><label><span className="mb-1 block text-xs text-slate-500">Dinheiro recebido</span><NumberInput className="input text-lg font-black" min={cashAmount} step={0.01} value={cashReceived} onValueChange={setCashReceived} /></label><div><p className="text-xs text-slate-500">Troco</p><p className="mt-2 text-3xl font-black text-lime">{currency(change)}</p></div></div>}
          <button className="btn-lime h-14 w-full text-base" disabled={completing || !cart.length || Math.abs(paymentDifference) > 0.009 || (hasCash && cashReceived < cashAmount)} onClick={complete}>{completing ? "Processando..." : "Confirmar venda"}</button>
          <button className="btn-ghost w-full" onClick={() => { setPaymentOpen(false); focusScanner(); }}>Voltar ao PDV</button>
        </section>
      </div>
    </Modal>
