...
export default function InventoryPage() {
  ...
  const products = useMemo(() => state.products.filter((product) => product.kind !== "combo" && [product.name, product.barcode, product.sku, product.category].join(" ").toLowerCase().includes(query.toLowerCase())).slice(0, 30), [state.products, query]);
  ...
  return <>
    ...
    <div className="mt-6 grid gap-6 2xl:grid-cols-[1fr_500px]">
      <section className="space-y-4">
        <div className="panel p-5"><form onSubmit={(event) => { event.preventDefault(); processScan(); }}><span className="mb-2 flex items-center gap-2 text-sm font-bold"><ScanBarcode size={19} className="text-lime" /> Contar por bipagem</span><div className="grid gap-2 sm:grid-cols-[96px_1fr_auto]"><input className="input h-16 text-center text-xl font-black" type="number" min="1" value={scanQty} onChange={(event) => setScanQty(Math.max(1, Number(event.target.value) || 1))} /><div className="relative"><Barcode className="absolute left-4 top-1/2 -translate-y-1/2 text-brand" size={24} /><input ref={scannerRef} className="input h-16 pl-12 text-xl font-mono" value={scan} onChange={(event) => setScan(event.target.value)} placeholder="Bipe o produto" autoComplete="off" /></div><button className="btn-primary px-6">Contar</button></div></form><p className="mt-2 text-xs text-slate-500">Digite uma quantidade antes do bip para contar várias unidades de uma vez. Códigos de caixa respeitam o multiplicador cadastrado.</p></div>
