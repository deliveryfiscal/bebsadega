  ...
  return (
    <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); if (form.kind === "combo" && !form.comboItems.length) { window.alert("Adicione pelo menos um componente antes de ativar/salvar um combo."); return; } onSave({ ...form, id: product?.id, active: form.active ?? product?.active ?? true, price: Number(form.price), cost: Number(form.cost), stock: Number(form.stock), minStock: Number(form.minStock), bottleVolumeMl: form.kind === "volume" ? Number(form.bottleVolumeMl) : undefined, dosePrices: form.kind === "volume" ? form.dosePrices : undefined, comboItems: form.kind === "combo" ? form.comboItems : undefined }); }}>
      <div className="grid gap-4 md:grid-cols-2">
        <label className="md:col-span-2"><span className="mb-1.5 block text-sm font-semibold">Nome do produto</span><input className="input" required value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Ex.: Heineken Long Neck 330ml" /></label>
        <label><span className="mb-1.5 block text-sm font-semibold">Código de barras</span><input className="input" autoFocus={Boolean(initialBarcode)} value={form.barcode} onChange={(e) => set("barcode", e.target.value)} placeholder="Bipe ou digite o EAN" /><span className="mt-1 block text-[11px] text-slate-500">Pode ficar em branco no cadastro inicial e ser vinculado depois com o leitor.</span></label>
        ...
        <label className="md:col-span-2 flex items-center justify-between rounded-xl border border-line bg-white/[0.03] p-4"><div><span className="block text-sm font-semibold">Produto ativo no PDV</span><span className="mt-1 block text-xs text-slate-500">Desative enquanto o código, estoque, preço ou composição ainda estiverem sendo conferidos.</span></div><input type="checkbox" className="h-5 w-5 accent-fuchsia-500" checked={Boolean(form.active)} onChange={(e) => set("active", e.target.checked)} /></label>
        <label><span className="mb-1.5 block text-sm font-semibold">Preço de custo</span><input className="input" type="number" min="0" step="0.01" required value={form.cost} onChange={(e) => set("cost", Number(e.target.value))} /></label>
        <label><span className="mb-1.5 block text-sm font-semibold">Preço de venda</span><input className="input" type="number" min="0" step="0.01" required value={form.price} onChange={(e) => set("price", Number(e.target.value))} /></label>
        {form.kind !== "combo" && <><label><span className="mb-1.5 block text-sm font-semibold">Estoque atual</span><input className="input" type="number" min="0" step="1" value={form.stock} onChange={(e) => set("stock", Number(e.target.value))} /></label><label><span className="mb-1.5 block text-sm font-semibold">Estoque mínimo</span><input className="input" type="number" min="0" step="1" value={form.minStock} onChange={(e) => set("minStock", Number(e.target.value))} /></label></>}
      </div>
      <div className="rounded-xl border border-line bg-white/[0.03] p-3 text-xs text-slate-400">Leitores USB e Bluetooth que funcionam como teclado são aceitos. O leitor normalmente envia o código e a tecla Enter automaticamente.</div>
      <div className="flex justify-end gap-2"><button type="button" className="btn-ghost" onClick={onCancel}>Cancelar</button><button className="btn-primary">Salvar produto</button></div>
    </form>
  );
}
